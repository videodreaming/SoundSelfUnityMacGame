...
namespace Michsky.UI.Beam
{
    ...
    public class ModeSelector : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler, ISubmitHandler
    {
        ...
        void OnEnable()
        {
            ...
            {
                LocalizedObject tempLoc = null;

                if (disabledHeaderObj != null) { tempLoc = disabledHeaderObj.GetComponent<LocalizedObject>(); }
                if (tempLoc != null) { tempLoc.localizationKey = headerTitleKey; tempLoc.UpdateItem(); }

                if (normalHeaderObj != null) { tempLoc = normalHeaderObj.GetComponent<LocalizedObject>(); }
                if (tempLoc != null) { tempLoc.localizationKey = headerTitleKey; tempLoc.UpdateItem(); }

                if (highlightHeaderObj != null) { tempLoc = highlightHeaderObj.GetComponent<LocalizedObject>(); }
                if (tempLoc != null) { tempLoc.localizationKey = headerTitleKey; tempLoc.UpdateItem(); }
            }

            else 
            {
                if (disabledHeaderObj != null) { disabledHeaderObj.text = headerTitle; }
                if (normalHeaderObj != null) { normalHeaderObj.text = headerTitle; }
                if (highlightHeaderObj != null) { highlightHeaderObj.text = headerTitle; }
            }
        ...
        public void Initialize()
        {
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                ...
                GameObject itemGO = Instantiate(itemPreset.gameObject, new Vector3(0, 0, 0), Quaternion.identity);
                ...
                {
                    if (useLocalization && !string.IsNullOrEmpty(items[tempIndex].titleKey)) { tempTitleOutput = localizedObject.GetKeyOutput(items[tempIndex].titleKey); }
