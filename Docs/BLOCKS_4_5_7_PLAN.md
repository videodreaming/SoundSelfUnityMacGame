...
| 9c — `targetNextFundamental` slot (full spine) | 🔄 Chunks 1+2 DONE (`0dbde359`; playtest PASSED); **Chunk 3 (shadow-tracker) DONE — EditMode green 2026-06-08, committed `d3fbde97`**; **Chunk 4 (source-switch) next** | `0dbde359` (C1+C2), `d3fbde97` (C3) | **C1**: slot + `DirectorConsultPendingFundamental` (structural realized-effect) + `ApplyMasterFundamentalRaw`. **C2**: slot-based `AnnounceInputDrivenFundamental` (immediate/deferred + disabled-raw) + `RouteTrigger`/`Effects` + retire `directorStoredFundamental`. **C3**: split-gate shadow tracker — `SilentCommitInputDriven` (`FUND-SHADOW`), audible commit records `preferred[activeSource]`, warm-handoff `resetCharge:false` via `ShouldCleanSlate` (`FUND-HANDOFF`); `ShouldWarnBehindCurtainUpdate` dropped (lean). **C4 next**: source-switch flush/adopt/commit. Reuses `WhichTest` from 9a |

---

## Active work — what's left to do

> **Execution order is driven by [`BLOCKS_4_5_7_BUILD_CHECKLIST.md`](BLOCKS_4_5_7_BUILD_CHECKLIST.md)** (adopted 2026-06-05): 4e ✅ → the **Stage 9 goblin spine** (9a ✅; 9c Chunks 1+2 ✅ `0dbde359`; 9c Chunk 3 shadow-tracker ✅ `d3fbde97`; **9c Chunk 4 source-switch next**) → 4g / 4f → cleanups → musical polish → Lorna external. That checklist has the step-by-step boxes + the "how it works / why" primer; this section is the prose summary.
...
**▶ Now — 9c Chunk 4 (source-switch flush/adopt/commit). Chunk 3 (shadow-tracker) DONE: EditMode green 2026-06-08, committed `d3fbde97` (pushed). Goblin re-playtest steps 4–6 still recommended for perceptual sign-off.**
...
- **🔄 9c — Chunks 1+2 DONE (tests green + goblin playtest PASSED 2026-06-05).** **C1**: `targetNextFundamental` slot consulted in `ActivateQueue` via `DirectorConsultPendingFundamental` (structural realized-effect — a counted audio event exists only when the slot moved the master) + `ApplyMasterFundamentalRaw` (no self-clear). **C2**: slot-based `AnnounceInputDrivenFundamental` (immediate = set slot + disabled?raw:`ActivateQueue`; deferred = set slot only) + `TryApplyFundamentalChangeTriggers` routed through `RouteTrigger`/`Effects` + **`directorStoredFundamental` retired** (slot subsumes the dedupe) + B457 `FUND-SLOT` schema. **C3 (DONE — EditMode green 2026-06-08, committed `d3fbde97`; goblin re-playtest steps 4–6 still recommended)**: split the ladder gate so it runs behind the curtain; `SilentCommitInputDriven` (`FUND-SHADOW` — preferred + reset charge, no master/Director); audible commit records `preferred[activeFundamentalSource]` inside `ApplyMasterFundamentalRaw`; warm-handoff `ApplyMasterFundamental(…, resetCharge:false)` on InputDriven None-adopt driven by `FundamentalSourcePolicy.ShouldCleanSlate` (`FUND-HANDOFF`); goblin steps 4–6 + `DebugSimulateBehindCurtainSilentCommit`. `ShouldWarnBehindCurtainUpdate`/`FUND-CURTAIN` **dropped** (lean — existing "stored, not written" log covers it). **C4 (next)**: source-switch flush/adopt/commit. *(Appendix G; design spec [`HANDOFF_InputDriven_preferred_shadow_tracker.md`](HANDOFF_InputDriven_preferred_shadow_tracker.md))*
...
- **⭐ 9 — Director ↔ fundamental "goblin" (the spine):** **9a** (long-test enqueue-then-activate + 5s flourish suppression) **committed `b4fe6020`, playtest PASSED**; **9c Chunks 1+2** (`targetNextFundamental` slot + `DirectorConsultPendingFundamental` + `ApplyMasterFundamentalRaw` + slot-based `AnnounceInputDrivenFundamental`/`RouteTrigger` + **structural** realized-effect + retire `directorStoredFundamental`) **committed `0dbde359`, playtest PASSED 2026-06-05**; **9c Chunk 3** (shadow-tracker preferred-sync + warm handoff + behind-the-curtain — the deferred 4e fix, Layer 1 of commit semantics) **DONE — EditMode green 2026-06-08, committed `d3fbde97`**; **next = Chunk 4** (source-switch flush/adopt/commit). *(Appendix G)*
