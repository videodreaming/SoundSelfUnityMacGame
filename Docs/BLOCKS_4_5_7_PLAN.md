...
| 9a — long-test flourish fix + 5s anti-clutter | ✅ code+tests in (`b4fe6020`, pushed); **playtest PASSED 2026-06-05** | `b4fe6020` | `AnnounceInputDrivenFundamental` enqueue-then-activate + disabled-bypass; `FundamentalTriggerPolicy.WhichTest` (parity anchor) + `FundamentalDirectorPolicy.FlourishDecision`/`ShouldAddFlourish`; dedicated `timeSinceLastFlourish`; B457 logs (FUND-ANNOUNCE/COMMIT, DIRECTOR-FLOURISH); F-key `Playground_Debug` playtest. **Playtest pass (B457 log + by ear/eye):** Step 1 one change → one `add=visual`; Step 2 burst 5× `FUND-COMMIT` / 1× `add=visual` / 4× `suppressed`; Step 3 disabled → `path=raw`, no flourish. |
| 9c — `targetNextFundamental` slot (full spine) | ✅ **DONE** — Chunks 1+2 (`0dbde359`; playtest PASSED); Chunk 3 shadow-tracker (`d3fbde97`); **Chunk 4 source-switch (playtest PASSED 2026-06-09)** — goblin spine complete | `0dbde359` (C1+C2), `d3fbde97` (C3), `ee86bd7d` (C4) | **C1**: slot + `DirectorConsultPendingFundamental` (structural realized-effect) + `ApplyMasterFundamentalRaw`. **C2**: slot-based `AnnounceInputDrivenFundamental` (immediate/deferred + disabled-raw) + `RouteTrigger`/`Effects` + retire `directorStoredFundamental`. **C3**: split-gate shadow tracker — `SilentCommitInputDriven` (`FUND-SHADOW`), audible commit records `preferred[activeSource]`, warm-handoff `resetCharge:false` via `ShouldCleanSlate` (`FUND-HANDOFF`); `ShouldWarnBehindCurtainUpdate` dropped (lean). **C4**: `SetFundamentalSource` flush+adopt+commit → switch that moves the master is a Director beat (one flourish) via `SwitchCommitDisposition` {None\|Raw\|Director}; `nextFundamentalResetsCharge` companion (switch commits resetCharge:false, sung change true); `Director.IsActivatingQueue` re-entrancy guard (shuffle-driven switch commits raw, no nested `ActivateQueue`). Reuses `WhichTest` from 9a |

---

## Active work — what's left to do

> **Execution order is driven by [`BLOCKS_4_5_7_BUILD_CHECKLIST.md`](BLOCKS_4_5_7_BUILD_CHECKLIST.md)** (adopted 2026-06-05): 4e ✅ → the **Stage 9 goblin spine** (9a ✅; 9c ✅ — Chunks 1–4 done, playtest PASSED 2026-06-09) → **4g / 4f** → cleanups → musical polish → Lorna external. That checklist has the step-by-step boxes + the "how it works / why" primer; this section is the prose summary.
...
**▶ Now — 4g (MusicBed `Cue_Key_*` listener) / 4f (`HarmonyRunPolicy`) — first consumers of the finished goblin spine. Stage 9 goblin **complete** (9a + 9c Chunks 1–4, playtest PASSED 2026-06-09).**

- **✅ 9a — committed `b4fe6020` (pushed); PERCEPTUAL PLAYTEST PASSED 2026-06-05.** Long-test enqueue-then-activate (`AnnounceInputDrivenFundamental`) + disabled-bypass; `FundamentalTriggerPolicy.WhichTest` (parity anchor, production routed through it) + `FundamentalDirectorPolicy.FlourishDecision`/`ShouldAddFlourish`; dedicated `Director.timeSinceLastFlourish` (~5s); B457 logs (`FUND-ANNOUNCE`/`FUND-COMMIT`/`DIRECTOR-FLOURISH`); EditMode tests; `F`-key guided goblin playtest (now driven by the temporary `DebugSimulateSungFundamentalChange` simulator — see [Appendix I](#appendix-i--live-test-playtest-design-conventions)). **Playtest result (B457 log + by ear/eye, all pass):** Step 1 one change pairs one `DIRECTOR-FLOURISH add=visual`; Step 2 burst = 5× `FUND-COMMIT`, 1× `add=visual`, 4× `suppressed` (5s gate); Step 3 disabled Director → `FUND-ANNOUNCE path=raw` + `FUND-COMMIT`, no flourish. **Next action:** build 9c.
- **✅ 9c — DONE (Chunks 1–4; EditMode green + goblin playtest PASSED 2026-06-09).** **C1**: `targetNextFundamental` slot consulted in `ActivateQueue` via `DirectorConsultPendingFundamental` (structural realized-effect) + `ApplyMasterFundamentalRaw`. **C2**: slot-based `AnnounceInputDrivenFundamental` + `RouteTrigger`/`Effects` + **`directorStoredFundamental` retired** + B457 `FUND-SLOT` schema. **C3** (`d3fbde97`): shadow tracker — `SilentCommitInputDriven` (`FUND-SHADOW`), `preferred[activeSource]` on audible commit, warm-handoff via `ShouldCleanSlate` (`FUND-HANDOFF`). **C4** (`ee86bd7d`): `SetFundamentalSource` flush+adopt+commit — switch that moves the master is a Director beat via `SwitchCommitDisposition`; `nextFundamentalResetsCharge` companion; `Director.IsActivatingQueue` re-entrancy guard (mid-activation switch commits raw). **Playtest (2026-06-09):** full goblin 9a+9c (steps 1–6) + 9C-C4 A/B; Step B re-run deterministic (`SetSoundscape(ShiftingEarth)` mid-activation) — one `[9C-C4] SetSoundscape`, `FUND-COMMIT A#→C src=MusicBed` raw (no `FUND-SLOT cleared → applying`), ≤1 flourish, no freeze. *(Appendix G)*
...
- **⭐ 9 — Director ↔ fundamental "goblin" (the spine):** **✅ DONE** — **9a** `b4fe6020` (playtest PASSED 2026-06-05); **9c** Chunks 1+2 `0dbde359`, Chunk 3 `d3fbde97`, Chunk 4 source-switch `ee86bd7d` (playtest PASSED 2026-06-09). Goblin spine complete — next work sits on this contract (4g/4f). *(Appendix G)*
...
| `b4fe6020` | **Stage 9a — route InputDriven fundamental change through the Director (long-test flourish fix) + 5s flourish anti-clutter** — `AnnounceInputDrivenFundamental(target, immediate)` (enqueue counted `fundamentalChange` + activate iff immediate; raw disabled-bypass); long/longish/short ladder routed through it. `FundamentalTriggerPolicy.WhichTest` (parity-anchor characterization of the ladder, production routed through it) + `FundamentalDirectorPolicy.FlourishDecision`/`ShouldAddFlourish`; dedicated `Director.timeSinceLastFlourish` gates the flourish add. B457 logs (`FUND-ANNOUNCE`/`FUND-COMMIT`/`DIRECTOR-FLOURISH`). EditMode: `Block7FundamentalTriggerPolicy` + `Block7FundamentalDirectorPolicy` + `F`-key case. Editor `F`-key guided goblin playtest in `Playground_Debug`. Opus regression pass: parity confirmed. **Perceptual playtest PASSED 2026-06-05** (B457 log: Step 1 one `add=visual`; Step 2 5× `FUND-COMMIT`/1× `add=visual`/4× `suppressed`; Step 3 disabled → `path=raw`, no flourish). *(also carries the pending plan/checklist edits that recorded 4e)* |
| `0dbde359` | **Stage 9c Chunks 1+2 — the `targetNextFundamental` slot goes live (Director consult + slot-based trigger paths).** **C1**: added the slot + `DirectorConsultPendingFundamental` (consulted at the top of `ActivateQueue`; structural realized-effect — counts one audio event only when the slot actually moves the master; slot-aware empty-queue early-return) + `ApplyMasterFundamentalRaw` (master-apply primitive without the `ClearQueueOfType("fundamentalChange")` self-clear). **C2**: `TryApplyFundamentalChangeTriggers` routed through the pure `FundamentalTriggerPolicy.RouteTrigger`/`Effects` (active-writer rows wired; behind-the-curtain `SilentCommit` rows exist + unit-tested but gated off until C3); `AnnounceInputDrivenFundamental` is now slot-based (immediate = set slot + `disabled?raw:ActivateQueue`; deferred = set slot only); **`directorStoredFundamental` retired** (slot subsumes the short-test dedupe) + dead `Action_ChangeFundamental` removed; B457 logs migrated to the `FUND-SLOT` schema. EditMode: `Block7FundamentalTriggerPolicyEditModeTests` +13 (`RouteTrigger` ×9, `Effects` ×4), all green. **Goblin playtest PASSED 2026-06-05** (B457 log: Step 1 two-line `FUND-SLOT` set→cleared→`FUND-COMMIT`→`add=visual`; Step 2 5× `FUND-COMMIT`/1× `add=visual`/4× `suppressed`; Step 3 disabled → `FUND-SLOT …raw apply`→`FUND-COMMIT`, no flourish; no slot leak). Temporary editor-only playtest scaffolding retained (final-commit cleanup). C3 (shadow-tracker) + C4 (source-switch) remain. *(committed by Robin; this row + the status/checklist edits ride the next chunk's commit per the no-hash-only-commit rule.)* |
| `d3fbde97` | **Stage 9c Chunk 3 — shadow-tracker preferred-sync + warm handoff (behind-the-curtain commit semantics, Layer 1; the deferred 4e fix).** Split the ladder gate so it runs behind the curtain; `SilentCommitInputDriven` (`FUND-SHADOW` — records `preferred` + resets per-note charge, no master/Director write); audible commit records `preferred[activeFundamentalSource]` inside `ApplyMasterFundamentalRaw`; warm-handoff `ApplyMasterFundamental(…, resetCharge:false)` on InputDriven None-adopt driven by `FundamentalSourcePolicy.ShouldCleanSlate` (`FUND-HANDOFF`). `ShouldWarnBehindCurtainUpdate`/`FUND-CURTAIN` dropped (lean). Editor goblin steps 4–6 + `DebugSimulateBehindCurtainSilentCommit` scaffolding (final-commit cleanup). **EditMode green 2026-06-08** (full suite). Goblin re-playtest steps 4–6 recommended for perceptual sign-off. C4 (source-switch flush/adopt/commit) next. |
| `ee86bd7d` | **Stage 9c Chunk 4 — source-switch flush/adopt/commit (goblin spine complete).** `SetFundamentalSource` rewritten as flush (`ClearQueueOfType` + slot clear) → adopt (`ShouldCleanSlate` charge reset) → commit via `FundamentalSourcePolicy.SwitchCommitDisposition` ({None\|Raw\|Director}): a switch that moves the master is a Director beat (one flourish); disabled-bypass + benign same-note → raw re-post; `nextFundamentalResetsCharge` companion so switch commits preserve charge (warm handoff); `Director.IsActivatingQueue` re-entrancy guard (mid-activation source switch commits raw, no nested `ActivateQueue`). `FundamentalSwitchCommit` enum + 5 EditMode cases in `Block7FundamentalPolicyEditModeTests`. Goblin playtest harness: 9C-C4 steps + temporary F-key Step-B-only run. **EditMode green + goblin playtest PASSED 2026-06-09** (full 9a+9c steps 1–6; 9C-C4 A benign no-flourish; 9C-C4 B deterministic `SetSoundscape(ShiftingEarth)` mid-activation — `FUND-COMMIT A#→C src=MusicBed` raw, no `FUND-SLOT cleared → applying`, ≤1 flourish, no freeze). |
...
- **Fields stay declared in the core `MusicSystem1.cs`** (the compact field block) — partials share all members, so nothing needs accessors. The apply path (`ChangeFundamental` / `SetFundamentalDirect` / locks / `ResetInputDrivenFundamentalTimers` / `ResolveFundamentalOnUnlock`) stays in the core file (it becomes `ApplyMasterFundamental` in 4d).
...
>
> **Shadow-tracker `preferred`-sync is no longer a 4e item — it is DEFERRED to 9c** (a standalone attempt failed; it's Layer 1 of the goblin commit semantics). Spec consumed by 9c: [`HANDOFF_InputDriven_preferred_shadow_tracker.md`](Archive/HANDOFF_InputDriven_preferred_shadow_tracker.md).
...
- Side effects (`ResetInputDrivenFundamentalTimers`, `ClearQueueOfType`) must **not** fire on a silent preferred-only update.
...
**F3 — Re-entrancy is benign for 9a.** `ActivateQueue` iterates a **copy** (`queuedItems`) and `queue.Clear()`s at the end; the executed action's inner `ClearQueueOfType("fundamentalChange")` only touches the live dict (harmless to the in-flight copy). So 9a does **not** strictly need the `Raw`/announce split to be correct — but introducing a tiny `AnnounceInputDrivenFundamental(target, immediate)` helper now (raw-when-disabled; else enqueue, activate iff immediate) is the right seed and lets long + short share one path.
...
2. `activeFundamentalSource = newSource`; adopt intent = `firstFundamental` (real) or `preferred[newSource]`; for InputDriven + real note, clean-slate `ResetInputDrivenFundamentalTimers()`.
...
| **Longish** (`…−5` + just-activated, immediate branch) | **Silent commit** — `preferred = changeTarget` + `ResetInputDrivenFundamentalTimers()`; no master/Director | Master adopts `preferred` = longish target **directly**; charge ~0 → ladder quiet. Clean. |
...
**Apply primitive still needed:** `ApplyMasterFundamentalRaw(note)` = today's `ApplyMasterFundamental` **without** the `ClearQueueOfType("fundamentalChange")` self-clear (the Director owns the queue during activation / the bypass doesn't need it). Keep `ResetInputDrivenFundamentalTimers` (charge reset on commit is current behavior).

**Commit semantics — two commit forms.** A commit always records **`preferred[activeSource] = note`** + `ResetInputDrivenFundamentalTimers()`, *unless* it's a **debug-override** apply (the override sits on top and must not overwrite the source's real preferred). Two forms:
- **Audible commit** (active source, Director enabled or via the raw-bypass): `ApplyMasterFundamentalRaw(note)` = master + Wwise + binaural + `ResetInputDrivenFundamentalTimers` + `preferred[activeSource]=note`. Covers the **long-test immediate**, the **deferred short-test fire**, and the **disabled raw-bypass** uniformly, so `preferred` is always the last actually-committed note and the warm-handoff restore is correct. Charge reset here is parity with legacy (legacy reset at the long-test apply and when the queued short-test action ran — at fire, not at queue).
- **Silent commit** (InputDriven shadow-tracker behind the curtain — Robin 2026-06-05): `preferred[InputDriven] = changeTarget` + `ResetInputDrivenFundamentalTimers()` **only** — no master/Wwise/binaural/Director. This is what a behind-the-curtain **long test** does so `preferred` tracks while inactive.
...
- **Drive the real production path, not a bypass.** The simulator calls the same method the real input would (e.g. `DebugSimulateSungFundamentalChange` → `AnnounceInputDrivenFundamental`), so every gate/side-effect/log is identical to the real event. A shortcut that skips gates proves nothing.
