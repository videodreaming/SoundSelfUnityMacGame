...
**▶ Now — 9c Chunk 3 (shadow-tracker) CODE-COMPLETE 2026-06-05 (pending EditMode run + goblin re-playtest steps 4–6); next is 9c Chunk 4 (source-switch).**
...
- **🔄 9c — Chunks 1+2 DONE (tests green + goblin playtest PASSED 2026-06-05).** **C1**: `targetNextFundamental` slot consulted in `ActivateQueue` via `DirectorConsultPendingFundamental` (structural realized-effect — a counted audio event exists only when the slot moved the master) + `ApplyMasterFundamentalRaw` (no self-clear). **C2**: slot-based `AnnounceInputDrivenFundamental` (immediate = set slot + disabled?raw:`ActivateQueue`; deferred = set slot only) + `TryApplyFundamentalChangeTriggers` routed through `RouteTrigger`/`Effects` + **`directorStoredFundamental` retired** (slot subsumes the dedupe) + B457 `FUND-SLOT` schema. **C3 (CODE-COMPLETE 2026-06-05, pending EditMode + goblin re-playtest)**: split the ladder gate so it runs behind the curtain; `SilentCommitInputDriven` (`FUND-SHADOW` — preferred + reset charge, no master/Director); audible commit records `preferred[activeFundamentalSource]` inside `ApplyMasterFundamentalRaw`; warm-handoff `ApplyMasterFundamental(…, resetCharge:false)` on InputDriven None-adopt driven by `FundamentalSourcePolicy.ShouldCleanSlate` (`FUND-HANDOFF`); goblin steps 4–6 + `DebugSimulateBehindCurtainSilentCommit`. `ShouldWarnBehindCurtainUpdate`/`FUND-CURTAIN` **dropped** (lean — existing "stored, not written" log covers it). **C4**: source-switch flush/adopt/commit. *(Appendix G; design spec [`HANDOFF_InputDriven_preferred_shadow_tracker.md`](HANDOFF_InputDriven_preferred_shadow_tracker.md))*
