...
| 9c — `targetNextFundamental` slot (full spine) | 🔄 Chunks 1+2 DONE (`0dbde359`; tests green + goblin playtest PASSED 2026-06-05); Chunks 3 (shadow-tracker) + 4 (source-switch) remain | `0dbde359` (C1+C2) | **C1**: slot + `DirectorConsultPendingFundamental` (structural realized-effect) + `ApplyMasterFundamentalRaw`. **C2**: slot-based `AnnounceFundamental` (immediate/deferred + disabled-raw) + `RouteTrigger`/`Effects` + retire `directorStoredFundamental`. **C3/C4 next**: shadow-tracker preferred-sync/warm-handoff + source-switch flush/adopt/commit. Reuses `WhichTest` from 9a |
...
**▶ Now — 9c Chunks 1+2 in (tests green + goblin playtest PASSED 2026-06-05); next is 9c Chunk 3 (shadow-tracker) then Chunk 4 (source-switch).**
...
- **🔄 9c — Chunks 1+2 DONE (tests green + goblin playtest PASSED 2026-06-05).** **C1**: `targetNextFundamental` slot consulted in `ActivateQueue` via `DirectorConsultPendingFundamental` (structural realized-effect — a counted audio event exists only when the slot moved the master) + `ApplyMasterFundamentalRaw` (no self-clear). **C2**: slot-based `AnnounceFundamental` (immediate = set slot + disabled?raw:`ActivateQueue`; deferred = set slot only) + `TryApplyFundamentalChangeTriggers` routed through `RouteTrigger`/`Effects` + **`directorStoredFundamental` retired** (slot subsumes the dedupe) + B457 `FUND-SLOT` schema. **C3 (next)**: shadow-tracker `preferred`-sync + warm handoff + behind-the-curtain (`SilentCommit` rows + `ShouldWarnBehindCurtainUpdate`/`ShouldCleanSlate` + `FUND-SHADOW`/`FUND-HANDOFF`/`FUND-CURTAIN`). **C4**: source-switch flush/adopt/commit. *(Appendix G; design spec [`HANDOFF_InputDriven_preferred_shadow_tracker.md`](HANDOFF_InputDriven_preferred_shadow_tracker.md))*
...
- **⭐ 9 — Director ↔ fundamental "goblin" (the spine):** **9a** (long-test enqueue-then-activate + 5s flourish suppression) **committed `b4fe6020`, playtest PASSED**; **9c Chunks 1+2** (`targetNextFundamental` slot + `DirectorConsultPendingFundamental` + `ApplyMasterFundamentalRaw` + slot-based `AnnounceFundamental`/`RouteTrigger` + **structural** realized-effect + retire `directorStoredFundamental`) **committed `0dbde359`, playtest PASSED 2026-06-05**; **next = 9c Chunk 3** (shadow-tracker preferred-sync + warm handoff + behind-the-curtain — the deferred 4e fix, Layer 1 of commit semantics), **then Chunk 4** (source-switch flush/adopt/commit). *(Appendix G)*
