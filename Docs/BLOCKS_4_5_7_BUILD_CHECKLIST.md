...
- [x] **Source switch = flush + adopt + commit (uniform, no Sequence carve-out):** ① `ClearQueueOfType("fundamentalChange")` + `slot=null`; ② set active source, adopt `firstFundamental` (real) or `preferred[new]`, clean-slate reset only for InputDriven+real note; ③ commit via `SwitchCommitDisposition` (raw if `director.disable` / `IsActivatingQueue` / benign same-note; else slot + `ActivateQueue`). *(Chunk 4 — `nextFundamentalResetsCharge` companion; `Director.IsActivatingQueue` re-entrancy guard)*
...
- [ ] **Pure policies + EditMode** (the regression backbone — [Appendix G §"Test Runner tests"](BLOCKS_4_5_7_PLAN.md)):
  ...
  - [x] `FundamentalDirectorPolicy.FlourishDecision` + `ShouldAddFlourish` *(already landed 9a)*; `ShouldCleanSlate` **(Chunk 3 — 6 unit cases)**; `SwitchCommitDisposition` **(Chunk 4 — 5 unit cases in `Block7FundamentalPolicyEditModeTests`)**. `ShouldWarnBehindCurtainUpdate` **dropped** (lean — see above).
