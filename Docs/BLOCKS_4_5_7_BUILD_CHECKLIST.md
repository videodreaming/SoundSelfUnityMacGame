...
> ✅ **Gap closed in 9c Chunk 3 (code-complete 2026-06-05).** The ladder gate in `TryApplyFundamentalChangeTriggers` no longer blanket-early-returns when InputDriven isn't the active writer: it runs the full ladder behind the curtain and routes through `RouteTrigger`, so a behind-the-curtain long/longish does a **silent commit** (`SilentCommitInputDriven` → `preferred[InputDriven]` + reset charge, `FUND-SHADOW`), an **audible commit** records `preferred[activeFundamentalSource]` inside `ApplyMasterFundamentalRaw`, and the **adopt re-entry** (`SetFundamentalSource(InputDriven, None)`) honors-not-wipes via `ApplyMasterFundamental(…, resetCharge:false)` driven by `FundamentalSourcePolicy.ShouldCleanSlate` (`FUND-HANDOFF`). EditMode pins `ShouldCleanSlate`; the stateful behavior rides the goblin 9c playtest (steps 4–6). Design spec: [`HANDOFF_InputDriven_preferred_shadow_tracker.md`](HANDOFF_InputDriven_preferred_shadow_tracker.md) (consumed by 9c).
...
- [x] Introduce the seed helper `AnnounceInputDrivenFundamental(target, immediate)` (raw-when-disabled; else enqueue counted `fundamentalChange`, `ActivateQueue` iff immediate); route long/longish (immediate) + short (deferred) through it.
...
- [x] **Two trigger paths:** Immediate (long/longish) = set slot + (disabled→raw / else `ActivateQueue`); Deferred (short) = set slot, don't activate (next external beat picks it up). No "BlankAction" needed. *(Chunk 2 — `AnnounceInputDrivenFundamental` slot-based + `TryApplyFundamentalChangeTriggers` routed through `RouteTrigger`)*
...
- [x] **Shadow-tracker `preferred`-sync (Layer 1, the deferred 4e fix — §1.3):** split the ladder gate so charge/decision run whenever the **tracking gate** is open (the `!isActiveWriter` blanket early-return is gone); audible commit sets `preferred[activeFundamentalSource]=committedNote` (in `ApplyMasterFundamentalRaw`, keyed on the active source so a foreign commit can't stamp InputDriven); behind-the-curtain long/longish does a **silent commit** via `SilentCommitInputDriven` (`preferred[InputDriven]`+reset charge, no master/Director — `FUND-SHADOW`). *(Chunk 3)*
- [x] **Warm-handoff honor-not-wipe** on InputDriven re-entry: `SetFundamentalSource(InputDriven, None)` adopts `preferred` **without** `ResetInputDrivenFundamentalTimers()` (threaded as `ApplyMasterFundamental(…, resetCharge:false)`); clean-slate (reset) only on the real-note path. (`FUND-HANDOFF` log; EditMode `ShouldCleanSlate(InputDriven,None)==false`, `(InputDriven,realNote)==true`.) *(Chunk 3)*
  - **Scope decision (Robin 2026-06-05, "keep broad"):** `ShouldCleanSlate` is `false` for **every** non-InputDriven takeover too (MusicBed/Sequence via `SetFundamentalSource`), so a music-loop entry or a stage key-pin **no longer wipes** InputDriven's per-note charge (it did pre-9c). Intentional — most "instrument remembers your lean across loops/pins" feel; matches the HANDOFF doc formula; charge is ladder-only state so it can't leak elsewhere. *(Aside: `SetFundamentalForSource` — the 4g cue path — still resets via its default `resetCharge:true`; revisit when 4g wires cues.)*
- [x] ~~**Behind-the-curtain warning** (`ShouldWarnBehindCurtainUpdate`)~~ **DROPPED (lean-policy rule, 2026-06-05):** it was a single-expression predicate over the existing `ShouldWriteMaster`, and `SetFundamentalForSource` already logs the non-active "preferred stored, but not written (active=…)" case. No new policy/log added. *(Chunk 3)*
- [ ] **Pure policies + EditMode** (the regression backbone — [Appendix G §"Test Runner tests"](BLOCKS_4_5_7_PLAN.md)):
  ...
  - [x] `FundamentalDirectorPolicy.FlourishDecision` + `ShouldAddFlourish` *(already landed 9a)*; `ShouldCleanSlate` **(Chunk 3 — 6 unit cases in `Block7FundamentalPolicyEditModeTests`)**. `ShouldWarnBehindCurtainUpdate` **dropped** (lean — see above).
  - [~] Behind-the-curtain A/B/C case matrix — the **unit** rows (`RouteTrigger`/`Effects` Short-inert / Longish-Long silent-commit) landed in Chunk 2; the production split-gate + silent commit land in Chunk 3; the **log** assertions / reactivation-honor ride the goblin 9c playtest (steps 4–6).
- [~] **Verification logs (B457)** — `FUND-COMMIT` + `FUND-SLOT` live (Chunks 1+2); `FUND-SHADOW` (silent commit) + `FUND-HANDOFF` (warm re-entry) live (Chunk 3). `FUND-CURTAIN` **dropped** with `ShouldWarnBehindCurtainUpdate`.
- [~] **Playtest (headphones + lights):** audible change pairs one flourish ✅, rapid changes don't spam (5s) ✅, disabled-Director raw apply with no flourish ✅ *(goblin 9a PASSED 2026-06-05)*; **9c shadow steps 4–6 (behind-curtain silent+unlit + warm-handoff re-entry) added to the goblin — pending Robin's run.** Savasana C-pin / switch-away-and-back-no-stale-flourish remain for Chunk 4.
...
> **Scope (decided 2026-06-05):** land **9a minimal first** (disabled-bypass + 5s flourish + the `AnnounceInputDrivenFundamental` seed) as its own reviewable commit, **then 9c** (the full `targetNextFundamental` slot) on top. 9b is dropped (the slot retires `directorStoredFundamental`).

**Temporary playtest scaffolding — REMOVE at the Stage 9 final commit (Robin 2026-06-05):**

So the goblin/slot behaviors can be verified *without singing* (deterministic + measurable), the guided playtests drive **simulated** sung changes down the real `AnnounceInputDrivenFundamental` path — built per the live-test conventions in [Appendix I](BLOCKS_4_5_7_PLAN.md) (simulate the real path, anticipate→act→confirm, timer-baked waits, single `B457` filter). This is editor-only and re-introduces a "force the master" surface (the kind 4e deleted) **on purpose, temporarily** — it must all be deleted in one pass when Stage 9 is signed off:

- [ ] `MusicSystem1.DebugSimulateSungFundamentalChange(int semitoneOffset, bool immediate)` — production-file addition (`MusicSystem1.FundamentalAuthority.cs`, `#if UNITY_EDITOR`). Drives the real path (not a bypass): InputDriven write gate + Director counting/flourish + disabled-raw all still apply.
- [ ] `MusicSystem1.DebugSimulateBehindCurtainSilentCommit(int semitoneOffset)` — Chunk 3 addition (same `#if UNITY_EDITOR` block). Drives the real `SilentCommitInputDriven` path; refuses to run when InputDriven IS active (would be a real audible commit).
- [ ] Both `[B457 DEBUG-SIM]` log lines.
- [ ] `MusicDebugGuidedPlaytest`: the `SimSemitoneStep` / `RapidChangeBurstCount` / `RapidChangeSpacingSeconds` / `FlourishWindowClearSeconds` constants, the `WaitWithCountdown` helper, the sim calls in `RunGoblinCore`, **and the `RunGoblinShadowSection` (steps 4–6) 9c coroutine** (which calls `SetFundamentalSource(MusicBed/InputDriven)` directly + `DebugSimulateBehindCurtainSilentCommit`).
- [ ] **Add any further temporary debug here as 4g lands** so the final commit removes them together. *(Keep: the guided-playtest harness itself, the `B457` verification logs, and the EditMode policy tests — those are the permanent regression net.)*
