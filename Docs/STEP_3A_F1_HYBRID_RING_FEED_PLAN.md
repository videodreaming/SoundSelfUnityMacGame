...
> **Status (2026-05-06):** **Pass 1 committed.** **Pass 2 play-tested (Robin)** — overflow drops stay 0; coherent-pair fix shaved ~50 ms off the apparent gap; residual ~110–150 ms gap = mic-vs-engine clock skew (~0.025 %), self-limited by the `ReadRawSamples` 250 ms overflow guard. Drift bar (< 20 ms / 60 s) **passed** (~15 ms / 60 s). Gap-band bar **loosened to 40–180 ms** retroactively (40–90 was aspirational; clock skew sets the floor). **Ready to commit.** Pass 3 next.
>
