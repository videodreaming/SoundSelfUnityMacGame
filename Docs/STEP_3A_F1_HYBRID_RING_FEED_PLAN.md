...
> **Status (2026-05-06):** **Pass 1 + 2 committed.** **Pass 3 code landed** — deleted `audioThreadRing`/`audioRingWriteLock`/`audioRingWritePosition`/`audioRingWriteTotalSamples`/`audioRingWriteLastClipReadStart`/`Count`, `monoScratch`, `audioCapturePlayAlignmentBufferCount`, the dead `data[]→monoScratch` mono-mix block + audio-thread ring write block in `OnAudioFilterRead`, and `audioCallbackLockMissTotal` (snapshot + agg field too). **`FAIL_AUDIO_LOCK_CONTENTION` repointed** to read `aggRawRingReadLockMissTotal` (Decision 1A); priming-frames gate kept (Decision 2 = keep). **Awaiting 5-min Robin play test** (no regressions in any FAIL_*, no new clicks). Optional Pass 4 (GC threshold) still pending.
>
