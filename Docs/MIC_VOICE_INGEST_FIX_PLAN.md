...
- [x] All scenes load with no missing-script / missing-component warnings.
- [x] No `MicPipeline` **type** or `.cs` file in `Assets/` (repo-wide search: **zero** matches in `Assets/**/*.cs` as of 2026-05-08). Legacy plan text allowed `[FormerlySerializedAs("…MicPipeline…")]` strings — current codebase may have none; either outcome satisfies “no live `MicPipeline` dependency.”
- [x] Voice path works end-to-end (toning, monitoring, visuals, Wwise) — **identical** subjective experience to pre-merge. *(Narrative from 0.7d completion.)*
- [x] `MicVoiceIngestDebugAggregate` shows valid values for all existing sections.
- [x] Phase 1 FAIL OBSERVATION flags still fire on stuck spells (the bug is still reproducible after the merge — that's the proof we didn't accidentally hide it). *(Pre–audio-thread-ingest; superseded by Step 3+ architecture — historical.)*
- [x] No new audible clicks introduced.
- [x] No console warnings or errors on scene load.
...
- [x] Verify `aggAudioCallbackTotal` resumes climbing after a forced recovery (e.g. unplug / replug or programmatic restart). *Implementation landed with rebootstrap; **explicit unplug/replug Editor confirmation optional** before Step 5b if you want a fresh field log — not blocking.*
...
**Test (3a):**
- [x] Tone normally; pitch tracking responsive every frame. *(Step 3 closure + F1 hybrid ring-feed validation.)*
- [x] `toneActive` fires reliably (true on voicing, false on silence).
- [x] No regression in visuals or Wwise audio response (modulo interim unfiltered-feed period before 3b — judge "responsive" not "identical").
- [x] `aggImitoneInputAudioCallTotal` tracks `aggAudioCallbackTotal` 1:1 (or close to it; ratio ≥ `failImitoneFeedRatioMin`).
- [x] `aggImitoneGetStateCallTotal` climbs once per `Update()`.
- [x] Forced mic recovery: telemetry resumes climbing afterward (validates the prep fix). *(Rebootstrap code path; optional unplug confirmation noted above.)*
- [x] Profiler: no GC allocations on the audio thread. *(Step 1 / Step 3 verification narrative.)*
- [x] FAIL OBSERVATION: Phase 3 feed-side flags validated; **`FAILURE` may still go true on Phase 1 legacy-ingest flags until Step 5b** — same regression-oracle caveat as Step 1 test bar (not “false all session” for the composite boolean yet).
...
- [x] If `aggDbMicrophoneTearDetectedTotal` ever becomes non-zero during testing, escalate `_dbMicrophone` from `volatile` to `Interlocked.Exchange` and update the labels accordingly. ***N/A — stayed at 0 in 3b play-test; `volatile float` retained** (see Step 3b Developer notes).*
...
**Test (3b):**
- [x] Tone normally; pitch tracking still responsive every frame, `_dbMicrophone` updating smoothly. *(2026-05-06 play-test — Step 3b Developer notes.)*
- [x] **Click testing protocol** — run at Step 4/5a per deferred schedule; **3b-specific click protocol box ticked on Step 3 review + user subjective pass** (onset click deferred to 3c/F2 investigation). Full five-scenario appendix explicitly run again at **5a** (see Step 5a Developer notes).
- [x] `aggDbMicrophoneTearDetectedTotal` stays at 0 across a full session including loud / silent / rapid on-off transitions.
- [x] `aggDbMicrophoneSnapshot` matches the value the main thread is consuming (sanity).
- [x] `aggCrossThreadFieldsUsingVolatile` and `aggCrossThreadFieldsUsingInterlocked` populate sensibly (read them in the Inspector and confirm the lists match the actual code). *(Step 3 review pass verified label strings.)*
- [x] `MicVoiceIngestDebugAggregate` may show stale or weird values for legacy `unread_zero`-related fields — expected; cleaned up in Step 5b/6.
- [x] Profiler: no GC allocations on the audio thread.
- [x] FAIL OBSERVATION: Phase 3 atomicity + feed flags behaved per play-test; **`FAIL_AUDIO_GC_ALLOC_DETECTED` retired in 3b** — table in Step 6 still lists it historically; see retirement note there.
...
- **Production / soak `Debug.LogError` (post–Step 4 kickoff):** `MicVoiceIngestDebugAggregate` exposes `logFailObservationErrorsToConsole` (default **on**). When enabled: **rising edge** when composite `FAILURE` becomes true — one log line listing which `FAIL_*` flags fired plus `micExitReason`, rolling Hz, max gap ms, imitone/callback ratio, raw lock misses, overflow drops, `dbMic` snapshot, imitone state-stall frames. **First frame of each contiguous `_dbMicrophone` tear-detector streak** also logs (tear counter still increments every bad frame; log is streak-gated to avoid spam). **`imitone.InputAudio` deferred exception** (main-thread drain) upgraded from `LogWarning` to `LogError` with `[MicVoiceIngest]` prefix. Disable the aggregate toggle if another system ingests Unity logs and the volume is unwanted.

*Soft pass (2026-05-06):*
- **Status: Step 4 SOFT PASS — moving to Step 5a.** Subjective: "the test feels great, I'm not seeing any issues." Production logging in place (above bullet). The full **click testing protocol (all 5 scenarios)** is deliberately *deferred* to Step 5a's test phase rather than run here, because 5a (`feat: harden DirectVoiceMonitoring against clicks (M1/M2/M5/M6)`) is what addresses the click sources — running the protocol *before* 5a would either pass spuriously or surface clicks that 5a is about to fix. The Inspector bar (rolling Hz, max gap, tear total, FAILURE = false, imitone/callback ratio ~1, lock misses ~0) was healthy across all subjective testing.
- Extended-testing checklist boxes in this section deliberately left **unticked** — full A/B against Step 0 baseline + Profiler pass + edge-case sweep can be revisited if a regression appears. Step 5a's test phase doubles as the deeper Step 4 verification (the click protocol covers most of the edge cases, and the M1/M2/M6 mitigations are exactly what stresses the underflow / overflow / gain-step paths).
- Optional V10 DSP-buffer-size latency tuning: not exercised; latency feels acceptable on current setting. Listed as an open knob in the lever inventory (Stage 5) for later.
...
- [x] **USER VERIFIED IN EDITOR (2026-05-06):** `ImitoneVoiceIntepreter` and `DirectVoiceMonitoring` are on **separate** GameObjects (per V5), each with **exactly one** AudioSource. Multiple AudioSources on the same GameObject would reintroduce `OnAudioFilterRead` chain-routing ambiguity and break the architecture's correctness guarantees.
- [x] **USER VERIFIED IN EDITOR (2026-05-06):** Project Settings → Script Execution Order: `ImitoneVoiceIntepreter` (capture, at −104) runs **before** `DirectVoiceMonitoring` (at −102). Unity fires `OnAudioFilterRead` callbacks across MonoBehaviours in script-execution-order regardless of GameObject co-location, so this is sufficient to guarantee the ring is freshly written before monitoring reads it every callback. No drift since Step 0.

*Click prevention M1 (click-free underflow handling in `DirectVoiceMonitoring`):*
- [x] Audit `DirectVoiceMonitoring.OnAudioFilterRead`'s underflow / lock-contention path. — *Audited: helper `ReadRawSamples` / `ReadNormalizedSamples` zero-fills the destination tail when `copied < frameCount` (`MicIngest.cs:619–622`); `DirectVoiceMonitoring.OnAudioFilterRead` then multiplies by gain and emits, so the zero step from last real sample is the click source. Mitigation lives in `DirectVoiceMonitoring` so the imitone feed (also a 4-arg helper consumer) is unaffected.*
- [x] Replace the silence-fill — *Implemented user-chosen "**short fade on both ends**" variant: at underflow entry, linearly fade `lastRealSample → 0` over `clickMitigationFadeSamples` (default 32) starting at index `copied`, then 0-hold for the rest of the region (helper already zero-filled). At underflow exit (recovery, first non-underflow callback after a streak), linearly fade-in `0 → realSample` over the first 32 samples by multiplying `monitoringMonoReadBuffer[0..fadeLen-1]` by `t` ramp. Mid-buffer partial recoveries (`copied > 0` AND `wasInUnderflowLastBuffer`) handled generically — both fade-in (over `min(fadeLen, copied)`) and fade-out (over `min(fadeLen, frameCount-copied)`) apply in the same callback. Fade source: `monoBuffer[copied-1]` if any real sample arrived, else previous-callback's trailing emit (pre-gain).*
- [x] Document the chosen behavior in a header comment. — *Done: `OnAudioFilterRead` carries a full XML doc summary describing all 7 pipeline steps including M1 entry-fade and recovery-fade.*

*Click prevention M2 (click-free overflow handling in `DirectVoiceMonitoring`):*
- [x] Audit `DirectVoiceMonitoring.OnAudioFilterRead`'s overflow / sample-skip path. — *Audited: helper jumps `readPosition` forward when writes lap reads (`MicIngest.cs:594–610`). Destination buffer is contiguous post-jump; the perceptible discontinuity is between **previous OAFR's last emit** and **this OAFR's first sample**.*
- [x] Replace the hard cut with a short crossfade. — *Implemented: when `overflowDropped > 0` AND `audioThreadHasPreviousBuffer`, crossfade the first `clickMitigationFadeSamples` mono samples from `audioThreadLastEmittedMonoSample / max(prevGain, 1e-4f)` (previous trailing emit, pre-gain-undone) to the post-jump natural samples via `Mathf.Lerp`. Pre-gain-undo of `audioThreadLastEmittedMonoSample` keeps the crossfade in the same level space as `monoBuffer[i]`, which is then re-scaled by M6's per-sample gain in the write step.*
- [x] Verify `aggMonitoringOverflowTotal` still increments. — *Verified: `bufferOverflowDropCount` / `bufferOverflowDropCountWindow` / `bufferOverflowDropSamples` are still `Interlocked.Increment`'d / `Interlocked.Add`'d before the M2 crossfade runs. Mitigation makes the click inaudible, not the counter invisible.*

*Click prevention M5 (clip not reassigned mid-session):*
- [x] Audit every code path that touches `monitoringSource.clip`. — *Audited: pre-5a `ConfigureMonitoringSourceClip` was called from `SetupMonitoring` (startup), `Update` (on `MicCaptureEpoch` change), and `SetMonitoringStreamSource` (mid-session). Each runtime call assigned the streaming-mic clip, flushing the AudioSource playback head and re-engaging startup priming.*
- [x] After this sub-pass, `monitoringSource.clip` is assigned exactly once at startup and never reassigned. — *Implemented Option A (silent dummy clip), mirroring the imitone F1 hybrid pivot. `Awake` creates a silent in-memory `AudioClip.Create("DirectVoiceMonitoring-silent-cadence", sampleRate, 1, sampleRate, stream:false)` (1 s buffer, auto-zero-filled) and assigns it to `monitoringSource.clip`. `OnAudioFilterRead` overwrites `data[]` from the ring read, so the clip's sample data is never read — it only drives callback cadence. `OnDestroy` releases the dummy clip.*
- [x] Remove `ConfigureMonitoringSourceClip`-style mid-session reassignment paths. — *Renamed to `AcknowledgeMicCaptureReady` (validates interpreter ready, updates `lastSeenCaptureEpoch`; does **not** touch `monitoringSource.clip`). Update's epoch-change branch now just re-primes the buffered read cursor on a recovery — AudioSource is unaffected. `SetMonitoringStreamSource` no longer calls the function for the side-effect; just re-primes the cursor for the new ring.*

*Click prevention M6 (smooth gain interpolation):*
- [x] Audit how `DirectVoiceMonitoring` applies `monitoringVolume × dynamicScale`. — *Audited: pre-5a `OnAudioFilterRead` snapshotted `effectiveMonitoringGain` once and applied it as a constant across the entire callback buffer (step change at buffer boundary). `effectiveMonitoringGain` is volatile and updated on the main thread inside `ApplyMonitoringVolume` (which runs every `Update`, every `set_monitoring_volume`, every `attenuation_toggle`, and on every `OnValidate`).*
- [x] Change to per-sample interpolation. — *Implemented: each callback snapshots `currentGain = effectiveMonitoringGain`, lerps `gain = Lerp(audioThreadLastAppliedGain, currentGain, (frame+1)/frameCount)` across the buffer, multiplies the mono sample by `gain`, fans to channels, and stores `currentGain` as `audioThreadLastAppliedGain` for the next callback. First callback uses `previousGain = currentGain` (no ramp from stale 0). On disable / re-enable, audio-thread state is reset so the next emit doesn't ramp from a stale value.*
...
| **5b-vi** | §9 **kickoff:** CURRENT TEST + D9 observability → engineer runs click protocol → **closeout:** grep log + plan tick + optional chore commit. |
...
***5b-iv — Aggregate orphaned-mirror cleanup + CURRENT TEST kickoff (per §9).*** **Combined-commit shape adopted** (instead of two-commit kickoff/deletion split documented at top): orphan sweep came back empty (5b-ii caught everything), so commit B reduces to phantom-counter retirement only — folded into the kickoff commit per engineer direction.
- [x] **CURRENT TEST rewrite for 5b regression-watch.** Field set landed exactly as approved: `currentTestSessionTimeSeconds`, `currentTestFailure`, `currentTestMicExitReason` (new — F1 hybrid producer health), `currentTestMicRawRingWriteTotalSamples` (new — must keep climbing; main-thread producer is the writer under F1 hybrid), `currentTestImitoneInputAudioCallTotal` (new — audio thread still feeds imitone), `currentTestDbMicrophoneTearDetectedTotal` (3b regression watch), `currentTestAudioCallbackHzRolling`, `currentTestDbValue`, `currentTestPitchHz`, `currentTestMonUnderflowEvents` / `currentTestMonOverflowEvents` (5a click-mitigation regression watch). Two 5a-specific fields retired from CURRENT TEST: `currentTestMonStarvationEvents` and `currentTestMonHardVolumeStepCount` (their `agg*` source fields stay in the Monitoring transport section; CURRENT TEST just stops surfacing them per §9 "no accumulation"). Header text + giant tooltip block + `currentTestDescription` default string + LateUpdate mirror copy lines all rewritten in the same edit pass.
- [x] **Aggregate orphan sweep:** no orphans found. 5b-ii had already deleted the gentle-recovery mirrors; no new stragglers surfaced. Sweep recorded as complete-by-vacuity.
- [x] **Phantom-counter retirement (`FAIL_RING_OVERFLOW_GROWING` + `aggMicRingOverflowSkipTotal`):** documented as a placeholder for an architecture (audio-thread ring producer) the F1 hybrid pivot abandoned. Source counter `micRingOverflowSkipTotal` (in `ImitoneVoiceIntepreter.AudioThread.cs`) had **zero write sites** — counter never incremented; FAIL flag could never fire. Retirement covers: source counter field, snapshot-struct field on `AudioThreadHealthSnapshot`, aggregate `aggMicRingOverflowSkipTotal` mirror, `FAIL_RING_OVERFLOW_GROWING` field + tooltip, `failRingOverflowWindowSeconds` threshold, three internal trigger state vars (`_prevAggMicRingOverflowSkipTotal`, `_lastMicRingOverflowIncreaseRealtime`, `_failSeenRingOverflowGrowing`), the LateUpdate trigger block, the LateUpdate snapshot copy line, the false-branch reset, the `FAILURE` OR term, the soak-log accumulator + reset + two rendering strings, and the `aggCrossThreadFieldsUsingInterlocked` label-string mention. Net behavior change: **zero** (term in `FAILURE` was always `false`; nothing else consumed the counter). Three orphaned values left serialized in `Assets/Scenes/MainGame.unity` (will drop on next scene save, same handling as 5b-ii's scene orphans).
- [x] **Telemetry consolidation sweep (per §A.5):** verified aggregate remains canonical for cross-cutting metrics (FAIL flags, ring counters, audio-thread health, monitoring transport totals); `DirectVoiceMonitoring` keeps its self-concern fields (per-callback monitoring counters, click-mitigation knob); `ImitoneVoiceIntepreter` keeps `toneActive` / pitch / dB / breath-coroutine game-logic telemetry. No duplicate cross-cutting metrics found across files; aggregate is authoritative.
- [x] Compile clean (no linter errors on the three modified files).
- [x] Cross-Assets grep: only remaining `micRingOverflowSkipTotal` / `FAIL_RING_OVERFLOW_GROWING` / `aggMicRingOverflowSkipTotal` / `failRingOverflowWindowSeconds` mentions in `Assets/**/*.cs` are intentional retirement-marker comments. Scene file `Assets/Scenes/MainGame.unity` still contains 3 orphaned serialized property values — Unity drops these on next scene save.

***5b-v — `[DefaultExecutionOrder(50)]` removal from `ImitoneVoiceIntepreter`.***
- [x] **USER ACTION REQUIRED:** open Editor → Project Settings → Script Execution Order. Confirm `ImitoneVoiceIntepreter` is listed at **−104**. If missing, **abort this pass** and flag for engineer. *Engineer verified 2026-05-08 ("Script execution order is AOK") — `ImitoneVoiceIntepreter` at -104 confirmed; other voice-path scripts unchanged from 5b-i baseline.*
- [x] Once confirmed, delete the `[DefaultExecutionOrder(50)]` line at the top of `ImitoneVoiceIntepreter.cs`. Replaced with a short retirement-marker comment pointing at Project Settings as the binding source.
- [x] Verify no other voice-path file carries a `[DefaultExecutionOrder(...)]` — `DirectVoiceMonitoring`, `RecordedAudioPlayback`, `MicVoiceIngestDebugAggregate` all have **none today** (verified 2026-05-08); nothing to do for those.
- [x] Compile clean (no linter errors on `ImitoneVoiceIntepreter.cs`).
- [x] Cross-Voice grep: only remaining `DefaultExecutionOrder` mention in `Assets/Scripts/Voice/**/*.cs` is the retirement-marker comment.

***5b-vi — Final grep sweep + click protocol.*** Split per §9: **kickoff commit** (`CURRENT TEST` rewrite first), **closeout commit** after engineer runs click protocol.

***5b-vi kickoff (§9 — code).***
- [x] Rewrite `MicVoiceIngestDebugAggregate.CURRENT TEST` for combined **5-scenario click protocol + F1 hybrid regression bar** (D8): 13 fields — sessionTime, failure, micExitReason, micRawRingWriteTotalSamples, imitoneInputAudioCallTotal, dbMicTearTotal proxy (`currentTestDbMicrophoneTearDetectedTotal`), rolling Hz, dbValue, pitchHz, monUnderflowEvents, monOverflowEvents, **monStarvationEvents**, **monHardVolumeStepCount** (two restored from 5a protocol visibility).
- [x] **D9(a):** tighten `currentTestMicExitReason` tooltip — `unread_zero` explained as chunked `Microphone.GetPosition()` / unread-sample-count semantics; not "between voice frames."
- [x] **D9(b):** surface `aggUnreadZeroConsecutiveFrames` (`[SerializeField]` in Mic ingest section; LateUpdate copy from `_consecutiveUnreadZeroFrames` after FAIL phase-1 streak logic).
- [x] Compile clean (`MicVoiceIngestDebugAggregate.cs` — no linter errors).
- [x] Commit shape: `chore(step5b-vi-kickoff): CURRENT TEST + unread-zero streak visibility for click protocol` (or engineer-preferred message).

***5b-vi mechanical grep (agent — may precede or accompany closeout).***
- [x] `gentleUnreadZero|PerformGentleUnreadZeroCaptureRestart|debugGentleUnreadZero` → **zero matches** in `Assets/**/*.cs` (verified 2026-05-08).
- [x] `FAIL_GENTLE_RECOVERY_FIRED` → **only** `//` retirement comments (3 sites in `MicVoiceIngestDebugAggregate.cs`; verified 2026-05-08).
- [x] `DefaultExecutionOrder` under `Assets/Scripts/Voice/**/*.cs` → **only** retirement-marker comment in `ImitoneVoiceIntepreter.cs` (verified 2026-05-08).

***5b-vi click protocol + FAIL sanity (engineer — gate before closeout commit).***
- [x] Full 5-scenario click protocol passes (quiet baseline, sustained tone, onset/offset, heavy load, long session). **Pass criterion:** no audible clicks; M1/M2/M6 mitigations from 5a still hold subjectively. *Engineer verdict: PASS (2026-05-08).*
- [x] `FAILURE` stays `false` during normal operation across scenarios; `FAIL_UNREAD_ZERO_SUSTAINED` / `FAIL_INGEST_RING_STALLED` behave sensibly (expect occasional startup-related FAIL logs only if prior behavior unchanged).

***5b-vi closeout (agent — after engineer reports protocol PASS).***
- [x] Log click-protocol verdict + any notable counter deltas in Developer notes (5).
...
- `chore(step5b-iv): rewrite CURRENT TEST + retire phantom ring-overflow chain` (5b-iv; **combined commit** per engineer direction — orphan sweep came back empty so the kickoff/deletion split documented in §5b-iv was collapsed into one commit).
- `chore(step5b-v): remove [DefaultExecutionOrder(50)] (Project Settings −104 verified)` (5b-v).
- `chore(step5b-vi-kickoff): CURRENT TEST + unread-zero streak visibility for click protocol` (5b-vi kickoff per §9).
- `chore(step5b): final grep sweep + click protocol closeout` (5b-vi closeout — after engineer PASS).
...
*Kickoff + 5a (2026-05-06):*
- **Step 4 closed as soft pass** (subjective verdict + production logging); full click protocol deliberately deferred to 5a's test phase since 5a fixes the click sources. Step 5a kicked off on Opus 4.7 per recommended LLM matrix.
- **5a M1 (underflow fades, both ends):** user explicitly chose "**short fade on both ends**" over the simpler DC-hold (Option A) and the one-sided Option B. Implementation: at underflow entry, fade `lastRealSample → 0` over `clickMitigationFadeSamples` (default 32, range 8–128); at underflow exit (recovery), fade-in `0 → realSample` over the same length. Mid-buffer partial recoveries (`copied > 0` AND `wasInUnderflowLastBuffer`) get both fades in the same callback. Helper-level zero-fill is intentionally left intact — mitigation lives in `DirectVoiceMonitoring` so the imitone feed (also a 4-arg helper consumer) is unaffected.
- **5a M2 (overflow crossfade):** when the helper jumps the read cursor (`overflowDropped > 0`) and we have a previous emit to anchor against, crossfade the first 32 mono samples from `audioThreadLastEmittedMonoSample / max(prevGain, 1e-4f)` (pre-gain-undone) to the post-jump natural samples. The pre-gain undo is what lets the crossfade live cleanly above the M6 gain ramp without double-attenuation. `bufferOverflowDropCount` / `*Window` / `*Samples` still increment before the crossfade runs (counter visibility preserved).
- **5a M5 (silent dummy clip — recommended path; user delegated):** chose Option A (silent in-memory dummy clip) over Option B (gate streaming-mic clip to startup-only). Reasoning: (1) mirrors the imitone F1 hybrid pivot — same trick, two consumers, one mental model; (2) eliminates an entire bug category structurally (no reassignment path = no reassignment bugs); (3) clip data is unread anyway (OAFR overwrites `data[]`); (4) simplifies `SetMonitoringStreamSource` to just re-prime the cursor; (5) forward-compatible with 5b ring consolidation; (6) tiny cost (~4 KB silent float[]). Renamed `ConfigureMonitoringSourceClip` → `AcknowledgeMicCaptureReady` and stripped clip-reassignment from `Update` epoch-change branch and `SetMonitoringStreamSource`. `OnDestroy` cleans up the dummy clip on scene reload.
- **5a M6 (per-sample gain interpolation):** pre-5a applied a per-callback constant gain (step change at every buffer boundary). Now `Mathf.Lerp(audioThreadLastAppliedGain, currentGain, t)` per sample, with `t = (frame+1)/frameCount`. Cost is one lerp per sample on the audio thread; benefit is no clicks at `toneActive` flips, attenuation toggles, dynamic-volume ramps. Trailing gain stored for the next callback; reset to 0 on disable so the first re-enabled emit doesn't ramp from a stale value.
- **New audio-thread-only state in `DirectVoiceMonitoring`:** `audioThreadLastEmittedMonoSample`, `audioThreadHasPreviousBuffer`, `audioThreadWasInUnderflowLastBuffer`, `audioThreadLastAppliedGain`. All four are read+written exclusively inside `OnAudioFilterRead` — single-threaded access from the audio thread; **no `volatile` / `Interlocked` needed** (only `effectiveMonitoringGain` is the cross-thread reader, and it's already `volatile`). The aggregate's `aggCrossThreadFieldsUsingVolatile` label does **not** need to grow for these new fields — they're not cross-thread. New `[SerializeField] [Range(8, 128)] int clickMitigationFadeSamples = 32;` is the only Inspector-tunable knob added.
- **M3 (verify-only, USER ACTION REQUIRED before 5a click test):** open the Editor and confirm:
  1. `ImitoneVoiceIntepreter` and `DirectVoiceMonitoring` are on **separate** GameObjects, each with **exactly one** `AudioSource`.
  2. Project Settings → Script Execution Order: `ImitoneVoiceIntepreter` runs at `−104` and `DirectVoiceMonitoring` runs at `−102` (interpreter before monitoring). Drift here is the most likely path to a regression that 5a's crossfades **cannot** save us from — if monitoring runs before capture in a callback, the crossfade-from-previous-emit anchors against samples that are now older than the buffer being assembled.
  3. Tick both M3 boxes once verified, then move to the click protocol.
- **5a click protocol (run all 5 scenarios; this is also the deferred Step 4 click test):** quiet baseline (30 s silence), sustained tone (30 s steady note), onset/offset (rapid voice on/off — hardest test for M1 fade-in / fade-out), heavy load (tone + simulated CPU spike — hardest test for M2 overflow crossfade), long session (5+ min — confirms no slow degradation). Counters to spot-check during/after: `bufferUnderflowFillCount`, `bufferOverflowDropCount`, `callbackStarvationCount`, `hardVolumeStepCount` (M6 should keep this at 0 even on `toneActive` flips), `aggDbMicrophoneTearDetectedTotal` (regression watch from 3b).
- **Sub-pass 5a is structurally complete; 5b (delete legacy block) remains pending** and is the larger surgical pass. 5b needs an audit pass first (the legacy block lives in `ImitoneVoiceIntepreter.MicIngest.cs:54k chars / 1303 lines` post-merge — non-trivial; risk of "secretly load-bearing" telemetry per the plan's Notes & considerations). Run 5a click test, fix any surfaced issues, **then** kick off 5b.

*5a SOFT PASS — first attempt (2026-05-06):*
- **User verdict:** "M3 verified. Click protocol all good." All five scenarios passed subjectively. M3's two USER VERIFY boxes ticked above; click protocol task `[ ]` boxes left as-is in the appendix (separate-doc checklist).
- **Process miss flagged by user (own it):** the §9 standard says CURRENT TEST gets rewritten **before** any new testing round, so the relevant counters are front-and-center in the aggregate Inspector. This was not done for 5a — the click protocol counters (`bufferUnderflowFillCount`, `bufferOverflowDropCount`, `callbackStarvationCount`, `hardVolumeStepCount`) live in `DirectVoiceMonitoring`'s own Inspector, not in the aggregate, so the user couldn't see them in CURRENT TEST during the protocol. User passed the protocol subjectively (no audible clicks) without counter visibility.
- **Forward discipline (binding from now on):** **rewrite CURRENT TEST as the *first* action of every new sub-pass kickoff commit**, never as part of the implementation commit and never after-the-fact. The kickoff commit's shape is now canonically: (1) rewrite CURRENT TEST for the new sub-pass's verification bar, (2) plan dev-notes kickoff entry, (3) *then* a separate implementation commit.

*5a counter-visibility kickoff (retroactive §9 fix, 2026-05-06):*
- User correctly pushed back: "shouldn't I test with these counters? (I'm still not seeing them)." Applied the §9 standard retroactively to 5a — rewriting CURRENT TEST **before** the re-test rather than deferring to 5b.
- **Aggregate changes (this commit):** rewrote `MicVoiceIngestDebugAggregate.CURRENT TEST` block for Step 5a click protocol verification. Replaced the Step 4 verify+tune field set wholesale (per §9 "no accumulation"). New CURRENT TEST surfaces: `failure`, `dbMicTearTotal` (3b regression watch), `audioCallbackHzRolling` (audio thread alive precondition), `dbValue` + `pitchHz` (voice alive precondition), and the four monitoring counters that M1/M2/M5/M6 actually move — `monUnderflowEvents` (M1 trigger), `monOverflowEvents` (M2 trigger), `monStarvationEvents` (hard underflow / capture-not-feeding signal), `monHardVolumeStepCount` (M6 *input* signal — diagnostic, not pass/fail).
- **DirectVoiceMonitoring change:** added `public int HardVolumeStepCount => hardVolumeStepCount;` accessor so the aggregate can mirror the M6 diagnostic counter into CURRENT TEST. The other three monitoring counters (`bufferUnderflowFillCount`, `bufferOverflowDropCount`, `callbackStarvationCount`) were already exposed via the existing `GetBufferedTransportTotals` accessor — the aggregate already pulled them; they just weren't surfaced in CURRENT TEST.
- **Reframe (corrected during this kickoff):** my initial framing of `hardVolumeStepCount` as "M6 should keep this at 0 even on toneActive flips" was **wrong**. The counter detects main-thread per-frame gain deltas > 0.2 (the *input* to a click vector), not the audio-thread output. M6 smooths the audio-thread emit per sample so the input deltas become inaudible in the actual output. **Pass/fail for M6 is the user's ears, not this counter.** Counter climbing on `toneActive` flips / attenuation toggles is **expected** and shows M6 is doing its job. CURRENT TEST tooltip and the `HardVolumeStepCount` doc-comment both call this out explicitly.
- **Re-test in progress:** user re-runs the full 5-scenario click protocol with counter visibility now in CURRENT TEST. Previous subjective pass stands as a confidence anchor; this re-test confirms it with diagnostic context. Pass/fail criterion is unchanged: **no audible clicks across all 5 scenarios**; counters provide the "what M1/M2/M6 mitigated" story.

*5a counter-visible re-test PASS (2026-05-06):*
- **User verdict: 5a PASSED.** Three of five scenarios completed (silent baseline, sustained tone, intermittent burst tones), all subjectively click-free. Heavy-load and long-session scenarios deferred to ad-hoc verification — the architecture has held cleanly across 3/3 stress profiles, and the data below makes the remaining two scenarios a confidence check rather than a gate.
- **Steady-state counter behavior (gold-standard data):** monitoring counters froze in identical positions across all three sessions (`monUnderflowEvents = 34`, `monOverflowEvents = 2`, `monStarvationEvents = 31`, `monHardVolumeStepCount = 0`), session times 48 s → 85 s → 145 s. **Every M1 / M2 event happened in the startup transient (first ~2 s before capture catches the monitoring read cursor), then the system goes rock-solid even through arbitrary toning.** No ticks in steady state across 145 s.
- **M6 was not stress-tested:** `monHardVolumeStepCount = 0` even through intermittent burst toning means the dynamic-volume ramp speeds (`gameOnRiseSpeed`, `chargeRiseSpeed`, etc.) keep frame-to-frame deltas under the 0.2 threshold. M6 is load-bearing only when something *would* spike past 0.2; that's not happening in normal play. Architecture is correct; the verification just had nothing to verify against. M6 stays armed for any future scenario that exceeds the threshold.
- **Two startup `LogError`s observed and classified:**
  1. `FAIL_MONITORING_STARVATION_GROWING` at `hzRolling=15.4`, `dbMicSnap=-999.00` (pre-init sentinel) — exactly the 31 startup starvations registering before capture caught up. Known transient class (matches `monStarvationEvents` startup behavior).
  2. `FAIL_AUDIO_CALLBACK_GAP_HIGH` at `maxGapMs=43.35` (~2.04× nominal of 21.3 ms), `imitoneStateStallFrames=62` — single brief inter-callback jitter spike on engine settling; main thread also briefly stalled in the same gap. One-off blip; no recurrence.
- Both startup `LogError`s prompted a user-driven enhancement to the production logging contract: see "5a LogError sustained-edge upgrade" below.

*5a LogError sustained-edge upgrade (2026-05-06):*
- **User observation:** "for these observations to be useful for bug reports from the field, I should know how bad the failure was well into the failure, not at the start, so I know if there was a really long issue, for example." Strongly endorsed — one-shot rising-edge logging hides the most important signal (duration). A 0.1 s startup blip and a 60 s mid-session degradation produce the *same* one-line log entry under the original design.
- **New contract (this commit):** `MicVoiceIngestDebugAggregate.LateUpdate` now drives a **rising / sustained / falling** state machine for `FAIL_OBSERVATION` logging when `logFailObservationErrorsToConsole = true`:
  - **Rising edge:** log immediately with current-frame snapshot (unchanged behavior).
  - **Sustained:** re-log every `failObservationLogIntervalInitialSeconds` (default 0.25 s) with **elapsed-failure-time prefix + current-frame snapshot**, doubling the interval each emit (exponential backoff) up to `failObservationLogIntervalCapSeconds` (default 16 s). For a 60 s sustained failure that's ~9 logs total instead of ~240 at fixed 0.25 s. Per-flag accumulation tracker OR's in any new flag that fires during the window.
  - **Falling edge:** log once with `cleared after {duration:F2}s — flags seen during window: {accumulated set}`. The "flags seen during window" set is distinct from the rising/sustained snapshots — it captures every FAIL_* flag that was true at *any* point in the window, even ones that came and went between samples.
- **Why exponential, not fixed?** User originally proposed fixed 0.25 s; I countered with exponential (start 0.25 s, cap 16 s) to balance "fast feedback for short blips" against "log spam on long failures." User picked exponential. Both knobs are configurable in the aggregate Inspector.
- **Disable path:** set `logFailObservationErrorsToConsole = false` (master toggle, unchanged) OR set `failObservationLogIntervalInitialSeconds = 0` to keep rising + falling but skip periodic re-log.
...
*5b-iv — Aggregate orphaned-mirror cleanup + CURRENT TEST kickoff + phantom counter retirement (2026-05-08, Opus 4.7):*
- **Files modified (3):** `Assets/Scripts/Voice/MicVoiceIngestDebugAggregate.cs` (CURRENT TEST rewrite + phantom-chain deletion + label-string update), `Assets/Scripts/Voice/ImitoneVoiceIntepreter.AudioThread.cs` (source-counter field + snapshot copy line deletion), `Assets/Scripts/Voice/ImitoneVoiceIntepreter.cs` (snapshot-struct field deletion). Lints clean on all three.
- **Commit shape adjustment:** the original §5b-iv plan called for two commits (kickoff CURRENT TEST + deletion). Engineer authorized **combining them** because the orphan sweep came back empty (5b-ii caught everything in its own pass), so commit B's only meaningful work was the phantom-counter retirement — folding it into the kickoff commit was tidier without compromising the §9 kickoff discipline (CURRENT TEST is still rewritten as the *first* substantive change in the commit, before any deletion).
- **CURRENT TEST rewrite (kickoff portion):** field set landed exactly as approved (+3 / −2 / 8 kept = 11 fields total). Three new fields surface F1-hybrid health: `currentTestMicExitReason` (string — producer health), `currentTestMicRawRingWriteTotalSamples` (long — main-thread writer must climb at sample rate), `currentTestImitoneInputAudioCallTotal` (long — audio-thread consumer must climb 1:1 with audio callbacks). Two 5a-specific fields retired from CURRENT TEST per §9 "no accumulation": `currentTestMonStarvationEvents` and `currentTestMonHardVolumeStepCount` (their `agg*` source fields stay in the Monitoring transport section — only the CURRENT TEST mirror was dropped). Header text + giant tooltip + `currentTestDescription` default string + LateUpdate mirror copy lines all rewritten in the same pass; class-level summary doc-comment intentionally left alone (still accurately describes the file's purpose; tracking every CURRENT TEST rewrite there would be noise).
- **Phantom counter safety assessment (per engineer direction "do an assessment that it is genuinely safe to delete it before you do"):** source counter `micRingOverflowSkipTotal` had **zero write sites** in `Assets/**/*.cs` (full grep) — only an `Interlocked.Read` inside `GetAudioThreadHealthSnapshot`. The FAIL flag `FAIL_RING_OVERFLOW_GROWING` could therefore never fire under any code path. **No cross-script consumers** of any surface (counter, snapshot field, aggregate mirror, FAIL flag, threshold, internal state) outside the aggregate itself. **No game-logic gates** on any of these surfaces; pure observability. Effect on `FAILURE` OR expression: identical (the term was always `false`, removing it changes nothing). Verdict: safe to delete. Engineer approved deletion before edits landed.
- **Surfaces deleted (phantom-chain retirement):** `micRingOverflowSkipTotal` field in `AudioThread.cs` (replaced with retirement comment); `MicIngestDebugSnapshot.micRingOverflowSkipTotal` struct field in `ImitoneVoiceIntepreter.cs` (replaced with retirement comment); `aggMicRingOverflowSkipTotal` mirror + tooltip in aggregate; `FAIL_RING_OVERFLOW_GROWING` field + tooltip; `failRingOverflowWindowSeconds` threshold; three internal trigger state vars (`_prevAggMicRingOverflowSkipTotal`, `_lastMicRingOverflowIncreaseRealtime`, `_failSeenRingOverflowGrowing`); the LateUpdate snapshot copy line; the LateUpdate trigger block (incl. its baseline init + advance branch); the false-branch reset (`FAIL_RING_OVERFLOW_GROWING = false;` outside the trigger gate); the OR term in `FAILURE = ...`; the soak-log accumulator (`if (FAIL_RING_OVERFLOW_GROWING) ...`) + reset (`_failSeenRingOverflowGrowing = false`) + two rendering strings (in `BuildFailObservationAccumulatedFlagsSummary` and `BuildFailObservationLogDetail`); and the mention inside the `aggCrossThreadFieldsUsingInterlocked` startup label string. **All four surface files compile clean.**
- **Telemetry consolidation sweep (per §A.5):** verified aggregate remains canonical for cross-cutting metrics; `DirectVoiceMonitoring` keeps self-concern fields; `ImitoneVoiceIntepreter` keeps game-logic telemetry. No duplicate cross-cutting metrics found across files. Sweep recorded as complete; nothing actionable.
- **Cross-Assets grep verification:** only remaining `micRingOverflowSkipTotal` / `FAIL_RING_OVERFLOW_GROWING` / `aggMicRingOverflowSkipTotal` / `failRingOverflowWindowSeconds` / `_failSeenRingOverflowGrowing` mentions in `Assets/**/*.cs` are intentional retirement-marker comments (one in each of the three files modified). Zero behavioral references survive.
- **Scene-file orphans (not addressed in code pass):** `Assets/Scenes/MainGame.unity` still serializes the 3 deleted properties (`FAIL_RING_OVERFLOW_GROWING: 0`, `failRingOverflowWindowSeconds: 2`, `aggMicRingOverflowSkipTotal: 0`). Same handling as 5b-ii — Unity drops these on next scene save.
- **Plan §6 / §7 forward-looking docs:** the Field reference table (§6) row for `aggMicRingOverflowSkipTotal` and the FAIL row for `FAIL_RING_OVERFLOW_GROWING`, and the Step-7 ring-flow checklist mention, are marked **(retired in 5b-iv)** rather than deleted — preserves the historical reference for top-down readers while making the retirement explicit. The historical Step 3a record (the §3 narrative that documents why the counter was wired) is left intact; retirement is a new fact, not a rewrite of history.
- **Smoke-test status:** **deferred to engineer** — code edits + lints clean, but no Unity Play-mode smoke test was run from this agent context. Engineer should compile + run + glance at the new CURRENT TEST block (must show all 11 fields populated; must NOT show `monStarvation` / `monHardVolumeStep` rows; FAIL panel must show `FAIL_RING_OVERFLOW_GROWING` retired) before authorizing 5b-v.
- **5b-iv smoke test result (2026-05-08):** engineer confirmed "test is good" before authorizing 5b-v. **Bonus diagnostic finding during the smoke test:** engineer observed `aggMicExitReason = "unread_zero"` for ~30 frames at a stretch despite `failure = false` and the mic feeling responsive. Squared as **bursty driver behavior**: `Microphone.GetPosition()` updates in chunks (~6-tick granularity at the engineer's framerate, per `aggMicStalledWriteHeadFrames` oscillating 0-6), so most ticks see `unread_zero` but every ~6 ticks a `copied_samples` tick interrupts the streak — `_consecutiveUnreadZeroFrames` never reaches the 30-frame threshold. F1 hybrid's audio-thread ring consumer smooths the bursty producer, which is why imitone feel stays smooth. **Follow-ups (a)+(b)** from that diagnostic — tooltip tightening + Inspector surfacing of the unread-zero streak counter — **landed in 5b-vi kickoff** (see below).

*5b-vi kickoff — CURRENT TEST + D9 observability (2026-05-08):*
- **Scope:** §9 kickoff commit **before** engineer re-runs the full 5-scenario click protocol (D10). Combines D8 (13-field CURRENT TEST: F1 hybrid rows retained + `currentTestMonStarvationEvents` / `currentTestMonHardVolumeStepCount` restored from 5a visibility), D9(a) (`currentTestMicExitReason` tooltip — chunked `Microphone.GetPosition` / unread-sample semantics; no "between voice frames"), D9(b) (`aggUnreadZeroConsecutiveFrames` in Mic ingest section + LateUpdate mirror after FAIL phase-1 streak logic).
- **Files modified (1):** `Assets/Scripts/Voice/MicVoiceIngestDebugAggregate.cs` only. No compiler/linter errors.
- **Mechanical grep (same agent session, logged for 5b-vi closeout):** `gentleUnreadZero|PerformGentleUnreadZeroCaptureRestart|debugGentleUnreadZero` → zero matches in `Assets/**/*.cs`; `FAIL_GENTLE_RECOVERY_FIRED` → only `//` retirement comments in `MicVoiceIngestDebugAggregate.cs`; `DefaultExecutionOrder` under voice → only retirement marker in `ImitoneVoiceIntepreter.cs`.
- **Engineer next:** run full click protocol (quiet baseline 30 s, sustained tone 30 s, onset/offset, heavy load, long session 5+ min); same pass criterion as 5a — **no audible clicks**, `FAILURE` stays false. Report verdict → agent ticks 5b-vi closeout + optional `chore(step5b): …` commit. *Completed — see "5b-vi click protocol PASS + closeout" below.*

*5b-vi click protocol PASS + closeout (2026-05-08):*
...
- **Net behavior change:** zero. The Project Settings entry has been the binding source since at least Step 5a (Script Execution Order entries override `[DefaultExecutionOrder]` attributes in Unity); the attribute was misleading dead metadata. Project Settings entry → unchanged.
- **Lints:** clean.
- **Cross-Voice grep verification:** only remaining `DefaultExecutionOrder` mention in `Assets/Scripts/Voice/**/*.cs` is the retirement-marker comment in `ImitoneVoiceIntepreter.cs`. Zero behavioral references survive. The other three voice-path scripts (`DirectVoiceMonitoring.cs`, `RecordedAudioPlayback.cs`, `MicVoiceIngestDebugAggregate.cs`) were already attribute-free; no further action needed.
